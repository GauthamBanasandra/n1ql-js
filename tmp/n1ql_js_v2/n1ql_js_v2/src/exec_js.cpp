...
std::string V8Env::ExecJs(std::string js_source) {
  ...
  v8::Isolate *isolate = getIsolate();
  {
    ...
    v8::Local<v8::ObjectTemplate> global_functions =
    ...
    auto callback_mail =
        v8::FunctionTemplate::New(isolate, V8Env::SendMailFunction);
    global_functions->Set(name_mail, callback_mail);

//    Exposing the N1qlQuery() constructor to Js.
    auto name_constructor =
        v8::String::NewFromUtf8(isolate, "N1qlQuery");
    auto callback_constructor =
        v8::FunctionTemplate::New(isolate, V8Env::N1qlQueryConstructor);
    global_functions->Set(name_constructor, callback_constructor);

    // Create a new context.
    v8::Local<v8::Context> context =
        v8::Context::New(isolate, NULL, global_functions);
...
// This function accepts a function as argument and executes it as part of the
// parent script.
...
void V8Env::SendMailFunction(const v8::FunctionCallbackInfo<v8::Value> &args) {
  ...
  encoded_message += base64_encode(reinterpret_cast<const unsigned char *>(
                                       to_address_str.c_str()),
                                   to_address_str.length()) +
                     ";";
  encoded_message += base64_encode(reinterpret_cast<const unsigned char *>(
                                       subject_str.c_str()),
                                   subject_str.length()) +
                     ";";
  encoded_message += base64_encode(
      reinterpret_cast<const unsigned char *>(message_str.c_str()),
      message_str.length());

  FILE *in;
  if (!(in = popen("python /Users/gautham/projects/file_modifier/send_email.py",
                   "w"))) {
...
void V8Env::N1qlQueryConstructor(
  ...
  obj->Set(exec_query_name,
           v8::FunctionTemplate::New(isolate, V8Env::ExecQueryFunction));
  ...
  v8::Local<v8::String> stop_iter_name =
      String::NewFromUtf8(isolate, "stopIter");
  obj->Set(stop_iter_name,
           v8::FunctionTemplate::New(isolate, V8Env::StopIterFunction));

  // If the arguments to the constructor is non-empty, initialize the instance
  // with query string.
